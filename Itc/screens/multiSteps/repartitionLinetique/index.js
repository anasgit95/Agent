...
const MasqueMur: () => Node = ({ setActiveSteps }) => {
    ...
    async function fetchData() {
        try {
            const value = JSON.parse(await AsyncStorage.getItem('RepartitionLinetique'));
            if (value !== null) {
                setMur(value)
            }
            else setMur([])
...
source={require('../../../assets/hauteur.png')}
/>
<Image
style={styles.stretch}
source={require('../../../assets/langeur.png')}
/>
                <ScrollView
                    ...
                    <View
                        ...
                        <TouchableOpacity style={{width:"100%",marginLeft:"20%"}} onPress={
                            ...
                            <InputView>
                                <Text key={item.designation} style={{ paddingTop: 20, paddingBottom: 20, paddingLeft: 5 }}> Repartion {index+1}</Text>
                            </InputView>
                            </TouchableOpacity>
                        )}
